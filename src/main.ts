...
async function transcribeViaYoutubeTranscript(videoId: string): Promise<{ transcript: string; language?: string } | null> {
  try {
...
async function transcribeViaOpenRouter(
  ...
  log('info', `Large audio (${(audioBytes.length / 1024 / 1024).toFixed(1)} MB) — splitting into ${totalChunks} parts...`);
...
async function transcribeYoutubeUrl(
  ...
  try {
    const orResult = await transcribeViaOpenRouter(downloadResult.audioPath, openRouterApiKey, openRouterModel, language);
